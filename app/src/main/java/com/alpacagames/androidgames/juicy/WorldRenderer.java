...
import com.alpacagames.androidgames.framework.gl.Camera2D;
import com.alpacagames.androidgames.framework.gl.SpriteBatcher;
import com.alpacagames.androidgames.framework.gl.TextureRegion;
import com.alpacagames.androidgames.framework.impl.GLGraphics;
...
public class WorldRenderer {
    ...
    private void renderBob() {
        ...
        case Bob.BOB_STATE_FALL:
            keyFrame = Assets.bobFall;
            break;
        case Bob.BOB_STATE_JUMP:
            keyFrame = Assets.bobJump;
            break;
        case Bob.BOB_STATE_POWER:
         	keyFrame = Assets.bobPower;
         	break;
        ...
        batcher.drawSprite(world.bob.position.x, world.bob.position.y, side * 4/3, 4/3, keyFrame);
    }
    
    private void renderBob2() {
    	 TextureRegion keyFrame;
    	 keyFrame = Assets.poison;
    	 batcher.drawSprite(world.bob2.position.x, world.bob2.position.y, 10, 1, keyFrame);
    }
    ...
    private void renderItems() {
        ...
        for(int i = 0; i < len; i++) {
            ...
            batcher.drawSprite(star.position.x, star.position.y, 1, 1, Assets.star);
        }
        len = world.jellys.size();
        for(int i = 0; i < len; i++) {
            Jelly jelly = world.jellys.get(i);            
            batcher.drawSprite(jelly.position.x, jelly.position.y, 1, 1, Assets.jelly);
        }
    }
